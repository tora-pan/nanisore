...
/**
 * Test result reporter which supports detailed output of chai/jasmine/etc test results. Inspired by
 * code here: https://github.com/modernweb-dev/web/issues/229#issuecomment-732005741
 */
class SpecReporter {
  constructor() {
  ...
  outputSuite(suite, indent = "") {
    ...
    results +=
      suite.tests
        ...
        .join("\n") + "\n";
    if (suite.suites) {
      results += suite.suites.map((suite) => this.outputSuite(suite, indent + "  ")).join("\n");
    }
    return results;
  }
  ...
  specReporter({ reportResults = true } = {}) {
    return {
      onTestRunFinished: () => {},
      reportTestFileResults: async ({ logger, sessionsForTestFile, testFile }) => {
        if (!reportResults) {
          return;
        }
        const testReport = await this.generateTestReport(testFile, sessionsForTestFile);
        logger.group();
...
const defaultConfig = {
  ...
  browsers: [
    puppeteerLauncher({
      launchOptions: {
        executablePath: process.env.NANISORE_PUPPETEER_EXEC_PATH
          ? process.env.NANISORE_PUPPETEER_EXEC_PATH
